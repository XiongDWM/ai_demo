...
public class FiberTools {
    ...
    @Tool(name = "findCableIdByName", 
         description = "根据光缆名称查询光缆ID（编号/唯一标识符），如问题包含‘id是多少’‘编号是多少’等，返回-1表示查询失败。问题示例：‘光缆金牛局~110kV洞子口站48芯普通光缆01的id是多少？’")
    public String findCableIdByName(
            @ToolParam(required = true, description = "光缆名称") String fiberName) {
        WebClient des = WebClient.create("http://192.168.0.66:18888");        
        System.out.println("findCableIdByName: " + fiberName);
        return "1234512121";
        // return Mono.fromCallable(() -> {
        //     System.out.println("find cable thread info:"+Thread.currentThread().getName());
        //     return des.post()
        //         .uri("/mcp/fiber/getIdByName")
        //         .contentType(MediaType.APPLICATION_JSON)
        //         .bodyValue(Map.of("name", fiberName))
        //         .retrieve()
        //         .bodyToMono(String.class)
        //         .doOnNext(r -> System.out.println("收到业务端响应：" + r))
        //         .onErrorReturn("查询失败")
    ...
    public String locateGlitch(
        ...
        return "光缆"+fiberName+"距离高新局端"+distance+"米处故障的准确gps为：104.062728，30.63568596296296"+"且根据推断，故障类型可能是纤芯劣化";
    }
    ...
    // public List<Fiber> findCableByStation(@ToolParam(required = true, description = "站点名称") String stationName) {
    ...
    // public RoutePoint findStationByName(@ToolParam(required = true, description = "站点名称") String stationName) {
    ...
    // @Tool(name = "findCableBetweenStationUndirection", description = "查询两个站点之间的光缆信息（无向）")
    ...
    // @Tool(name = "findCableBetweenStationIndirection", description = "查询两个站点之间的光缆信息（有向）")
    // public List<Fiber> findCableBetweenStationIndirection(
    //         @ToolParam(required = true, description = "起始站名称") String fromStationName,
    //         @ToolParam(required = true, description = "终止站名称") String toStationName) {
    //     return fiberService.getCableBetweenStationIndirection(fromStationName, toStationName);
    // }

    // @Tool(name = "findCableByDateRangeClassifyByType", description = "根据日期范围查询光缆信息并按类型分类，用于分析数据之前查询")
    // public Map<String, Long> findCableByDateRangeClassifyByType(
    //         @ToolParam(required = true, description = "开始日期，格式：yyyy-MM-dd") String startDate,
    //         @ToolParam(required = true, description = "结束日期，格式：yyyy-MM-dd") String endDate) {

...
public class MultiModalService {
    ...
    public Mono<Document> captionImage(String imageUrl){
...
//                .enableThinking(true)
                ...
                .multiModel(true)
                .build();
        Flux<ChatResponse> stream = dashscope.stream(new Prompt(userMessage, options));
        return stream.map(chatResp -> {
                    String text = chatResp.getResult().getOutput().getText();
                    return text != null ? text.trim() : "";
                })
                .filter(StringUtils::isNotBlank)
                .reduce(new StringBuilder(), StringBuilder::append).map(stringBuilder -> {
                    String caption = stringBuilder.toString().trim();
                    Document doc = new Document("图片"+resource.getFilename()+"的内容："+caption);
